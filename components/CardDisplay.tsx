...
export const CardDisplay: React.FC<CardDisplayProps> = ({ card, isDraggable = false, onDragStart, onClick, className = '', isSelected = false }) => {
  const isAction = card.type === 'action';

  const baseClasses = 'w-16 h-24 rounded-lg shadow-lg flex items-center justify-center font-bold text-lg cursor-pointer transition-all duration-200';
